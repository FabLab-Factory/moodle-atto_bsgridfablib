...
@editor @editor_atto @atto @atto_bsgridfablib @_bug_phantomjs
Feature: Atto bsgridfablib
  To layout content in Atto, I need to insert bootstrap grids

  Background: Ensure grid button in toolbar
    Given the following config values are set as admin:
      | toolbar  | "bootstrap = bsgridfablib" | editor_atto  |
      | enabled_templates | col2,col3,col1x3,col3x1,col4,col6 | atto_bsgridfablib |

  @javascript
  Scenario: Create a 2 column grid
    Given I log in as "admin"
    And the following config values are set as admin:
      | toolbar  | bsgridfablib = bsgridfablib | editor_atto |
